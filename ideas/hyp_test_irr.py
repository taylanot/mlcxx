...
def sample(a=100, phi=np.pi, N=50, noise=True, sort=True, width=5,test=False):
    ...
    if sort:
        x = np.sort(x,axis=0)
        if noise:
            y = np.multiply(a, np.sin(x+phi)) + np.random.normal(0,0.01,(N,1))
        else:
...
noise = False
print("uninformative-noise:{}".format(noise))
...
for case in range(repeat):
    ...
    dataset = sample(a=a, phi=phi, N=Ntst,sort=True,noise=noise,test=False)
