...
def sample_test(a=100, phi=np.pi, N=50, noise=True, sort=True, width=5):
    ...
    if sort:
        x = np.sort(x,axis=0)
        if noise:
            y = np.multiply((a*x)**2, np.sin(12*x+phi)) + np.random.normal(0,0.5,(N,1))
        else:
...
model.fit_inter(x_train_single,y_train_single)
#model_base.fit_inter(x_train_single,y_train_single)
...
#plt.plot(x_train_single,np.multiply((a*x_train_single)**2, np.sin(12*x_train_single+phi)), ':', label='sine')
#plt.xlabel('x')
#plt.xlabel('y')
#plt.legend()
##tikzplotlib.save('phi_4.tex')
##plt.savefig('no_opt.pdf')
#plt.show()
