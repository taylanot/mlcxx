...
def my_config():
    ...
    numba_config2 = numba_config


# Here's a function that expects a typed-dict as the argument
@njit
def move(numba_config, numba_config2):
    # inplace operations on the arrays
    var = numba_config['std_y']
    var['m'] = np.array([10000000.], dtype='f8')
    return numba_config2, numba_config
# Call move(d) to inplace update the arrays in the typed-dict.

@ex.automain
def main(numba_config, numba_config2):
    print(move(numba_config, numba_config2))
