...
@njit
def move(numba_config, numba_config_runner):
    # inplace operations on the arrays
    numba_config['m'] = np.array([1000000000.], dtype='f8')
    return numba_config, numba_config_runner
# Call move(d) to inplace update the arrays in the typed-dict.

@ex.automain
def main(numba_config, numba_config_runner):
    print(move(numba_config, numba_config_runner))
