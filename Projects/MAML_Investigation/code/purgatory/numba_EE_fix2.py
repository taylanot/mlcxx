...
@nb.njit(cache=True)
def EE_Linear(a_dist, xtrn_dist, xtst_dist, model_tag, run_tag, numba_config, numba_config_runner):
    var = numba_config 
...
@nb.njit(cache=True)
def EE_Ridge(a_dist, xtrn_dist, xtst_dist, model_tag, run_tag, numba_config, numba_config_runner):
    var = numba_config 
    run = numba_config_runner
    overall_ee = [ ] 
...
@nb.njit(cache=True)
def EE_SGD(a_dist, xtrn_dist, xtst_dist, model_tag, run_tag, numba_config, numba_config_runner):
    var = numba_config 
    run = numba_config_runner
    overall_ee = [ ] 
    for alpha in var['lr']:
...
def main(seed, config, model_tag, run_tag, numba_config, numba_config_runner, SUPPORTED_MODELS):
    ...
    set_seed(seed)
    xtrn_dist = np.random.multivariate_normal(np.zeros(int(config['dim'])),np.eye(int(config['dim']))*config['b'], (int(config['Ntrn']*config['Nz'])))
    xtst_dist = np.random.multivariate_normal(np.zeros(int(config['dim'])),np.eye(int(config['dim']))*config['b'], (int(config['Ntst']*config['Nz'])))
    a_dist = np.random.multivariate_normal(np.zeros(int(config['dim']))*config['m'],np.eye(int(config['dim']))*config['c'], (int(config['Na'])))

    if model_tag == "Bayes":
        res = EE_Bayes(a_dist, xtrn_dist, xtst_dist, model_tag, run_tag, numba_config, numba_config_runner)
    if model_tag == "Linear":
        res = EE_Linear(a_dist, xtrn_dist, xtst_dist, model_tag, run_tag, numba_config, numba_config_runner)
    if model_tag == "Ridge":
        res = EE_Ridge(a_dist, xtrn_dist, xtst_dist, model_tag, run_tag, numba_config, numba_config_runner)
    if model_tag == "SGD":
        res = EE_SGD(a_dist, xtrn_dist, xtst_dist, model_tag, run_tag, numba_config, numba_config_runner)
