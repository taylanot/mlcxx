...
template<class T>
covmat<T>::covmat(const arma::mat& input1)
{
  create(input1, input1);
}

template<class T>
covmat<T>::covmat(const arma::mat& input1, const arma::mat& input2)
{
...
covmat<T>::covmat(const arma::mat& input1, Types&&... args)
{
  T kernel(args...);
  create(input1, input1);
}
...
template<class T>
void covmat<T>::create(const arma::mat& input1, const arma::mat& input2)
{
  matrix.resize(input1.n_rows, input2.n_rows);

  for (int i = 0; i < int(input1.n_rows); i++)
    {
      for (int j = 0; j < int(input2.n_rows); j++)
        {
          matrix(i,j) = kernel.Evaluate(input1.row(i),input2.row(j));
        }
    }
