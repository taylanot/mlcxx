...
class DATASET():
    ...
    """ Create DATASET for the ML activities with pytorch """
    
    def __init__(self, infile=None, outfile=None, indata=None, outdata=None, filter_nan=True, **kwargs):
        ...
        if infile != None and outfile != None:
            self.input = tensorize_DataFrame(read_pickle(infile)).T
            self.output = tensorize_DataFrame(read_pickle(outfile)).T

        ################################
        # If you want to read data from from an DATA object and 
        # it has a tensor attribute
        ################################
        if indata != None and outdata != None:
            self.input = indata.tensor
            self.output = outdata.tensor
        ...
        # Crop data
        ################################
        if 'inx' in self.kwargs:
            self.unscaled_input = self.input
            self.scale_input(method=self.kwargs['scale_input'])
        if 'outx' in self.kwargs:
            self.unscaled_output = self.output
            self.scale_output(method=self.kwargs['scale_output'])
