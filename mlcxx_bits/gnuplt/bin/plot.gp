...
do for [i=1:words(dark2_colors)] {
    set style line i lt 2 lc rgb word(dark2_colors, i) lw 2 pt 7 ps 1.5 
}

# Initialize the plot command
plot for [i=1:words(files)] word(files, i) binary format='%float64' using 1:2 with linespoints linestyle i title word(files, i)
