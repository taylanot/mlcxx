...
do for [i=1:words(files)] {
    stats word(files, i) binary format='%float64' using 1 nooutput
    maxs[i] = STATS_max
    mins[i] = STATS_min
...
plot for [i=1:words(files)] word(files, i) binary format='%float64' using (bin($1, bin_widths[i])):(1.0) smooth freq with boxes linestyle i title word(files, i)
