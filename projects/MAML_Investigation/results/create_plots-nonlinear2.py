...
def get_results(exp_dir=Experiment_Directory, model='Linear', run_tag='std_y', dim=1, Ntrn=1):
    ...
    tag = model+label_ext
    if model == "MAML" and run_tag == 'n_iter':
        return x, ys
    else:
        return x, y, tag
        ...
        for model in models:
            if model not in ex:
                if model != 'MAML':
                    ...
                    x,ys= get_results(model=model, run_tag=run_tag,dim=dim, Ntrn=Ntrn)
                    for i in range(4):
...
plot_result(id_=0,run_tag='n_iter', dim=1, Ntrn=10, ex=[],save=False, name='test')
plt.show()
