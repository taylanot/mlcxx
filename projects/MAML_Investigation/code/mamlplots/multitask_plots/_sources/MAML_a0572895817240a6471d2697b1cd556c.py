...
class multitask():
    def __init__(self, network, beta, task_dist, num_points_task,\
            num_task_sample, show_weights=False, plot=False):
        self.network = network
        self.pT = task_dist
        self.k = num_points_task
        self.num_task = num_task_sample
    ...
    def plot(self, epoch, res=100, path='results'):
        xp = torch.linspace(self.pT.domain_bounds[0],self.pT.domain_bounds[1],\
                res).reshape(-1,1)
        boundary_tasks = self.pT.task_def.boundaries(xp)

        plt.figure()
        plt.plot(xp, self.network(xp).detach().numpy(),color='blue',\
                label='prediction')
        plt.plot(xp, boundary_tasks[0],'--',color='red',label='bounds')
        plt.plot(xp, boundary_tasks[1], '--',color='red')
        plt.legend()
